...
class MemoryData(BaseModel):
    ...
    related_character_ids: List[str] = Field(
        default_factory=list, description="関連キャラクターのIDリスト"
    )
...
class LongTermCharacterData(BaseModel):
    ...
    experiences: List[ExperienceData] = Field(
        default_factory=list, description="過去の重要な経験のリスト"
    )
    goals: List[GoalData] = Field(
        default_factory=list, description="達成したい目標/願望のリスト"
    )
    memories: List[MemoryData] = Field(
        default_factory=list, description="特定の記憶のリスト"
    )
...
class GenericInterventionDetails(BaseModel):
    ...
    extra_data: Dict[str, Any] = Field(
        default_factory=dict, description="介入固有のデータ"
    )
...
class InterventionData(BaseModel):
    ...
    intervention_type: str = Field(
        description="介入の種類 (例: SCENE_SITUATION_UPDATE, REVELATION)"
    )
    intervention: Union[
        SceneUpdateDetails, RevelationDetails, GenericInterventionDetails
    ] = Field(description="介入の詳細情報（介入タイプに応じた構造）")
...
class TurnData(BaseModel):
    ...
    character_name: str = Field(
        description="行動したキャラクターの名前 (ログの可読性のため)"
    )
    think: str = Field(description="キャラクターの思考内容")
    act: Optional[str] = Field(
        None, description="キャラクターの行動内容 (行動しない場合はNone)"
    )
    talk: Optional[str] = Field(
        None, description="キャラクターの発言内容 (発言しない場合はNone)"
    )
...
class SceneLogData(BaseModel):
    ...
    interventions_in_scene: List[InterventionData] = Field(
        default_factory=list, description="この場面で発生したユーザー介入のリスト"
    )
    turns: List[TurnData] = Field(
        default_factory=list, description="この場面で実行された各ターンのリスト"
    )
