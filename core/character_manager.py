...
class CharacterManager:
    ...
    def load_character_data(self, character_id: str) -> None:
        ...
        """
        # 既にキャッシュにある場合は何もしない
        if character_id in self._immutable_cache and character_id in self._long_term_cache:
            return
        ...
        try:
            ...
            # その他予期せぬエラー
            raise CharacterManagerError(f"Unexpected error loading character data for ID: {character_id}: {e}") from e
    ...
    def update_long_term_context(self, character_id: str, new_long_term_data: LongTermCharacterData) -> None:
        """
        キャラクターの長期情報を更新する
