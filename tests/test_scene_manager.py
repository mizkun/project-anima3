...
def test_load_nonexistent_scene(scene_manager):
    ...
    # 存在しないファイルパスを指定
    non_existent_path = os.path.join("tests", "test_data", "scenes", "non_existent_scene.yaml")
...
def test_load_invalid_yaml(scene_manager):
    ...
    # 不正なYAMLファイルを指定
    invalid_yaml_path = os.path.join("tests", "test_data", "scenes", "S002_invalid_yaml.yaml")
...
def test_load_validation_error(scene_manager):
    ...
    validation_error_path = os.path.join("tests", "test_data", "scenes", "S003_validation_error.yaml")
...
def test_load_different_scene(scene_manager):
    ...
    scene_manager.load_scene_from_file(scene1_path)
    assert scene_manager.get_current_scene_info().scene_id == "S001"
    assert scene_manager.get_participant_character_ids() == ["mei_kinoshita_001", "rinko_kizuki_002"] 